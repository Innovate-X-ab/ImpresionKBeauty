    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        console.log('Debugging - Available categories:', 
          Array.from(new Set(allData.map((p: Product) => p.category)))
        );
        ...
        if (!Array.isArray(data)) {
          console.error('Invalid collection data format');
          setProducts([]);
          setBrands([]);
          return;
        }
        ...
        // Extract unique brands only if we have valid data
        const uniqueBrands = Array.from(
          new Set(
            data.filter(p => p && p.brand)
