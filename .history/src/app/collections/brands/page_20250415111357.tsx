...
import { ArrowRight, ArrowRightCircle, ArrowLeftCircle } from 'lucide-react';
...
export default function Brands() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        ...
        // Update featured brand logic
        const featuredBrands = brandsWithDetails.filter(brand => brand.featured);
        if (featuredBrands.length > 0) {
          setFeaturedBrand(featuredBrands[currentFeaturedIndex]);
        }
      } catch (error) {
    ...
    fetchBrands();
  }, [currentFeaturedIndex]); // Add currentFeaturedIndex as dependency

  const handleNextBrand = () => {
    const featuredBrands = brands.filter(brand => brand.featured);
    setCurrentFeaturedIndex((prev) => 
      prev === featuredBrands.length - 1 ? 0 : prev + 1
    );
  };
  ...
  const handlePrevBrand = () => {
    const featuredBrands = brands.filter(brand => brand.featured);
    setCurrentFeaturedIndex((prev) => 
      prev === 0 ? featuredBrands.length - 1 : prev - 1
    );
  };
    ...
    <PageLayout className="pt-0 pb-0"> {/* Remove top and bottom padding */}
      ...
      <div className="bg-gradient-to-r from-blue-50 to-pink-50">
        <div className="container mx-auto px-4 py-12 sm:py-16">
          ...
          {featuredBrand && (
            <div className="mb-16 bg-white rounded-lg shadow-sm overflow-hidden relative group">
              {/* Navigation Arrows */}
              <button
                onClick={handlePrevBrand}
                className="absolute left-2 top-1/2 -translate-y-1/2 z-10 p-2 bg-white/80 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white"
              ...
              <button
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 z-10 p-2 bg-white/80 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white"
                aria-label="Next brand"
              >
                <ArrowRightCircle className="w-6 h-6 text-gray-700" />
              </button>
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2">
                {brands.filter(brand => brand.featured).map((_, index) => (
                  <button
                    key={index}
          ...
          <div className="mt-20 text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
              <div className="p-5 rounded-lg bg-gradient-to-br from-pink-50 to-purple-50">
                <h3 className="text-lg font-medium mb-2 text-gray-600">Quality Assurance</h3>
              ...
              <div className="p-5 rounded-lg bg-gradient-to-br from-blue-50 to-teal-50">
                <h3 className="text-lg font-medium mb-2 text-gray-600">Ethical Sourcing</h3>
              ...
              <div className="p-5 rounded-lg bg-gradient-to-br from-yellow-50 to-orange-50">
                <h3 className="text-lg font-medium mb-2 text-gray-600">Innovation Focus</h3>
