...
// Keep importing the original AdminOrder type from @/types/order
import { AdminOrder, ShippingDetails, OrderStatus } from '@/types/order'; // Assuming OrderStatus is exported from types/order
...
// Define an intermediate type for the raw data structure expected from server actions
// This reflects dates as strings and prices/amounts as numbers, as handled by server-side serialization
interface RawOrderDataFromServer {
  id: string;
  createdAt: string; // Date as ISO string
  status: OrderStatus; // Use the imported OrderStatus enum/type
  ...
  // Include any other fields returned by the server actions if necessary
}
...
export default function OrderDetailClient({ orderId }: OrderDetailClientProps) {
  // State still uses the original AdminOrder type
  ...
  // Parameter type is now RawOrderDataFromServer, replacing 'any'
  const processOrderData = (rawData: RawOrderDataFromServer): AdminOrder => {
    // Prices are already numbers based on RawOrderDataFromServer, no need for Number() conversion here
    // Type for 'item' is inferred from rawData.orderItems, replacing 'any'
    ...
    // Convert date strings from rawData to Date objects for AdminOrder
    ...
    return {
      ...
      totalAmount: rawData.totalAmount, // Already number
      ...
      user: rawData.user
      // Final cast is still useful to assert the final structure matches AdminOrder
    } as AdminOrder;
  };
  ...
  useEffect(() => {
    async function fetchOrder() {
      try {
        ...
        // Fetch raw data from the server action, expecting RawOrderDataFromServer structure
        const rawOrderData = await getOrderDetails(orderId) as RawOrderDataFromServer; // Cast the result
        ...
        const processedOrder = processOrderData(rawOrderData);
  ...
  const handleConfirmStatusUpdate = async () => {
    ...
    try {
      // Call the server action, expecting RawOrderDataFromServer structure
      const rawUpdatedOrderData = await updateOrderStatus(orderId, statusToUpdate) as RawOrderDataFromServer; // Cast the result
      ...
      const processedUpdatedOrder = processOrderData(rawUpdatedOrderData);
