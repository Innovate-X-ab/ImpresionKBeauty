...
import { getOrderDetails, updateOrderStatus, sendCustomEmail } from '@/app/actions/admin/orders'; // Added sendCustomEmail
...
const formatDate = (dateString: string | Date | null | undefined): string => {
  try {
    ...
    if (!(date instanceof Date) || isNaN(date.getTime())) {
      console.warn('Invalid date received for formatting:', dateString);
      return 'Invalid Date';
    }
...
export default function OrderDetailClient({ orderId }: OrderDetailClientProps) {
    ...
    async function fetchOrder() {
      try {
        ...
        const formattedOrder: AdminOrder = {
          id: orderData.id,
          createdAt: orderData.createdAt ? new Date(orderData.createdAt) : new Date(0),
          ...
          paymentInfo: orderData.paymentInfo ? {
            ...
            paymentDate: orderData.createdAt
          } : {
            paymentId: orderData.stripeSessionId || 'N/A',
            paymentMethod: 'N/A',
            paymentStatus: 'N/A',
            paymentDate: orderData.createdAt || new Date(0)
          }
        };
  ...
  const handleOpenPreviewModal = (newStatus: OrderStatus) => {
    ...
    const templateData = {
        ...
        customerName: order.user.name || 'Customer', // Use customer name
        // Add other necessary data for templates
        trackingNumber: 'PENDING', // Placeholder, replace if available
    ...
    if (newStatus === OrderStatus.SHIPPED) {
      ...
      content = orderShippedTemplate(templateData); // Generate HTML content
    } else if (newStatus === OrderStatus.DELIVERED) {
      subject = `Your Order #${order.id} Has Been Delivered!`;
      content = orderDeliveredTemplate(templateData); // Generate HTML content
    }
  ...
  const handleConfirmStatusUpdate = async () => {
    ...
    setIsPreviewModalOpen(false); // Close modal first
    setUpdatingStatus(true);
    setError(null);

    try {
      const updatedOrderData = await updateOrderStatus(orderId, statusToUpdate); // This action now also sends the email

      if (!updatedOrderData || !updatedOrderData.id) {
         throw new Error('Failed to receive valid updated order data.');
      }

      // Reformat data after update
      ...
      const formattedOrder: AdminOrder = {
        ...
        createdAt: updatedOrderData.createdAt ? new Date(updatedOrderData.createdAt) : new Date(0),
        ...
        paymentInfo: updatedOrderData.paymentInfo ? {
            ...
            paymentMethod: 'N/A',
            paymentStatus: 'N/A',
            paymentDate: updatedOrderData.createdAt || new Date(0)
          }
      };

      setOrder(formattedOrder);
      alert(`Order status updated to ${statusToUpdate} and email notification sent.`); // Update alert message

    } catch (err) {
      console.error(`Error updating status to ${statusToUpdate}:`, err);
    ...
    return (
      <div className="flex flex-wrap gap-2">
        {currentStatus === 'PENDING' && (
          <button
            // Directly update status for PROCESSING, no preview needed usually
            onClick={() => handleConfirmStatusUpdate()} // Call confirm directly
        ...
        {currentStatus === 'PROCESSING' && (
          <button
            // Open preview modal for SHIPPED status
            onClick={() => handleOpenPreviewModal(OrderStatus.SHIPPED)}
            disabled={updatingStatus}
        ...
        {currentStatus === 'SHIPPED' && (
          <button
            // Open preview modal for DELIVERED status
            onClick={() => handleOpenPreviewModal(OrderStatus.DELIVERED)}
            disabled={updatingStatus}
        ...
        {!['CANCELLED', 'DELIVERED'].includes(currentStatus) && (
          <button
            // Directly update status for CANCELLED, maybe add a confirmation prompt later
            onClick={() => {
                if (window.confirm('Are you sure you want to cancel this order? This action might not send an email notification.')) {
                    handleConfirmStatusUpdate() // Call confirm directly
                }
            }}
            disabled={updatingStatus}
            ...
            <div className="bg-white shadow-sm rounded-lg overflow-hidden">
              ...
              <div className="divide-y divide-gray-200">
                {order.orderItems.map((item) => {
                  ...
                  } catch (e) { console.error('Error parsing product images:', e); }
                 ...
                 <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-lg bg-white p-6 text-left align-middle shadow-xl transition-all">
                   ...
                   <div className="mt-4 space-y-4">
                     ...
                     <div>
                       ...
                       <textarea
                         ...
                         placeholder="Enter email body as HTML..."
                       />
                     </div>
                   </div>
                   ...
                   <div className="mt-6 flex justify-end space-x-3">
                     ...
                     <button
                       ...
                       disabled={emailSending || !manualEmailSubject || !manualEmailBody}
                     >
