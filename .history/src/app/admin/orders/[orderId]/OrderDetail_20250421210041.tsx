...
const formatDate = (dateString: string | Date | null | undefined): string => {
  try {
    ...
    if (!dateString) {
      console.log('No date provided:', dateString);
      return 'No date';
    }
...
export default function OrderDetailClient({ orderId }: OrderDetailClientProps) {
  ...
  useEffect(() => {
    async function fetchOrder() {
      try {
        ...
        console.log("Raw order createdAt:", orderData.createdAt);

                // Validate that we have a date
                if (!orderData.createdAt) {
                  console.warn("Order is missing createdAt date!");
                }
        
        // Create payment info regardless of whether it exists in the response
        const paymentInfo = {
        ...
        console.log("Payment info date:", paymentInfo.paymentDate);
        
        // Format the order items to match AdminOrder type
        ...
        const formattedOrder: AdminOrder = {
          ...
          // Ensure shippingAddress is never null
          shippingAddress: orderData.shippingAddress || '{}',
          orderItems: formattedOrderItems,
          user: orderData.user,
          // Explicitly add paymentInfo here
          paymentInfo: paymentInfo
        };

        console.log("Formatted order createdAt:", formattedOrder.createdAt);
        console.log("Formatted date:", formatDate(formattedOrder.createdAt));
  ...
  const handleUpdateStatus = async (newStatus: OrderStatus) => {
    try {
      ...
      console.log("Updated order createdAt:", updatedOrderData.createdAt);
      
      // We need to reconstruct the payment info when updating the order
      const paymentInfo = {
      ...
      const formattedOrder: AdminOrder = {
        ...
        createdAt: updatedOrderData.createdAt ? new Date(updatedOrderData.createdAt) : new Date(),
      ...
      // Automatically send email notification about status change
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              {order.createdAt ? formatDate(order.createdAt) : 'No date available'}
            </p>
          </div>
