    ...
    async function fetchOrder() {
      try {
        ...
        const formattedOrder: AdminOrder = {
          id: orderData.id,
          // Cast status to ClientOrderStatus after fetching
    ...
    return (
      <div className="flex flex-wrap gap-2">
        {currentStatus === 'PENDING' && (
          <button
            // Directly update status for PROCESSING
            onClick={() => { setStatusToUpdate('PROCESSING'); handleConfirmStatusUpdate(); }} // Set status before confirming
                 ...
                 <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-lg bg-white p-6 text-left align-middle shadow-xl transition-all">
                   ...
                   <div className="mt-4 space-y-4">
                     ...
                     <div>
                       ...
                       <input type="text" id="manualSubject" value={manualEmailSubject} onChange={(e) => setManualEmailSubject(e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                     </div>
                     <div>
                       <label htmlFor="manualBody" className="block text-sm font-medium text-gray-700 mb-1">Body (HTML)</label>
                       <textarea id="manualBody" value={manualEmailBody} onChange={(e) => setManualEmailBody(e.target.value)} rows={10} className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm font-mono" placeholder="Enter email body as HTML..." />
                     </div>
                   </div>
                   <div className="mt-6 flex justify-end space-x-3">
