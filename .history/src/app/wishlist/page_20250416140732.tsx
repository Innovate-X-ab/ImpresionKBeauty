...
import { Product, WishlistItem } from '@/types/products';
...
export default function WishlistPage() {
  ...
  useEffect(() => {
    const formatWishlistItems = () => {
      return (wishlist as Product[]).map((item) => {
        // Handle images
        let formattedImages = '';
        try {
          if (Array.isArray(item.images)) {
            formattedImages = item.images[0];
          } else if (typeof item.images === 'string') {
            const parsed = JSON.parse(item.images);
            formattedImages = Array.isArray(parsed) ? parsed[0] : item.images;
          }
        } catch (e) {
          formattedImages = typeof item.images === 'string' ? item.images : '';
