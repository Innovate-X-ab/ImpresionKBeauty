...
export async function getAllOrders() {
  try {
    ...
    // Serialize any Decimal fields in the orders
    const serializedOrders = serializeDecimalFields(orders);

    // Make sure dates are properly serialized
    const ordersWithFormattedDates = serializedOrders.map(order => ({
      ...order,
