...
export async function getAllOrders() {
  try {
    ...
    // Log dates for the first order for debugging
    if (orders.length > 0) {
      console.log('First order date from DB:', orders[0].createdAt);
    }

    // Convert dates to ISO strings BEFORE serializing other fields
    const ordersWithISODates = orders.map(order => ({
      ...order,
      createdAt: order.createdAt.toISOString(),
      updatedAt: order.updatedAt.toISOString()
    }));

    // Now serialize any Decimal fields in the orders
    const serializedOrders = serializeDecimalFields(ordersWithISODates);
